...
-- 직원 테이블에서 급여(SALARY)가 5000 이하인 직원의 FIRST_NAME, SALARY 정보를 출력
SELECT FIRST_NAME, SALARY
  FROM EMPLOYEES
WHERE SALARY <= 5000;

-- 직원테이블에서 연봉이 100000 이상인 직원의 이름(FIRST_NAME)과 급여와 연봉의 정보를 출력
...
-- 직원테이블에서 직업 아이디가 IT_PROG가 아닌 직원의 정보를 출력
-- (FIRST_NAME, JOB_ID)
...
-- 직원테이블에서 EMAIL 정보가 SKING인 직원의 모든 정보를 출력
SELECT *
  FROM EMPLOYEES
 WHERE EMAIL = 'SKING';
 
-- 직원 테이블에서 입사날짜가 06/03/07에 잆사한 직원의 직원ID와 이름의 정보를 출력
SELECT EMPLOYEE_ID, FIRST_NAME, HIRE_DATE
  FROM EMPLOYEES
 WHERE HIRE_DATE = '06/03/07';


-- JOB_ID가 'ST_MAN'인 직원의 수는 몇 명인가?
-- 정답 >> ?
SELECT *
  FROM EMPLOYEES;
...
-- 논리 연산자의 사용 방법
-- 3. SELECT    조회하고 싶은 컬럼 이름1, 조회하고 싶은 컬럼 이름N ...
-- 1. FROM      테이블 이름
...
-- 부서ID가 100이거나 입사일이 06년 02월 02일 이후에 입사한 직원은 총 몇 명인가
-- 정답 >> 55
...
-- 부서ID가 100이거나 90인 직원 중에서
-- 직원ID가 101인 직원의 직원ID, FIRST_NAME, 연봉을 출력 * 단 연봉은 AnnSal
-- 정답 > 1명!
...
-- 우리 부서에 부서가 없는 직원이 총 몇 명이냐 그럼 그 직원의 이름을 출력해와라!
-- 부서아이디가 없는 직원의 이름은?
...
--> = + OR 결과 값과 같은 값을 출력
--> IN 연산자는 NULL 값을 제외하는 특징을 가지고 있음

-- 직원 테이블에서 부서ID가 30이거나 50이거나 90인 모든 직원의 정보를 출력
SELECT *
  FROM EMPLOYEES 
...
-- 직원 테이블에서 직업 아이디가 AV_VP 이거나 ST_MAN인 직원의 이름과 직업 아이디를 출력(IN 연산자활용)
SELECT FIRST_NAME, JOB_ID
  FROM EMPLOYEES 
...
-- NOT IN 에 NULL 값을 넣게 되면 아예 데이터가 출력이 안 된다!!
-- !=- + AND
...
-- 직원 테이블에서 급여가 10000 이상 20000이하의 급여를 받는 직원의 이름과 급여 정보 출력
-- 10000 ~ 20000 
SELECT FIRST_NAME, SALARY
  FROM EMPLOYEES
 WHERE SALARY >= 10000
...
-- 급여가 10000 이상 20000이하인 범위 내의 데이터를 출력
...
-- 이름 : FIRST_NAME
-- 직원 테이블에서 이름이 S로 시작하고 n으로 끝나는 직원의 이름 찾기
SELECT FIRST_NAME
  FROM EMPLOYEES
 WHERE FIRST_NAME LIKE 'S%n';

-- 직원 테이블에서 이름이 it으로 끝나고 총 4글자인 직원의 이름 찾기
SELECT FIRST_NAME
  FROM EMPLOYEES
 WHERE FIRST_NAME LIKE '__it';

-- 직원 테이블에서 이름의 두 번째 글자가 e인 직원의 이름 찾기
SELECT FIRST_NAME
  FROM EMPLOYEES
