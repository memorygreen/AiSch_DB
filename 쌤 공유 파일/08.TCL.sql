-- CHAPTER 08. TCL (Transaction Cotrol Language)
...
-- ROLLBACK  : 트랜잭션 취소, 마지막 COMMIT시점 까지만 복구가 가능
...
-- 접속을 종료하기 까지의 전체 기간을 의미
--> 세션이 여러개 라는 말은 오라클 데이터베이스에 접속하여 사용중인 연결이 
-- 여러개 있다는 뜻이다.

-- 읽기 일관성 이란?
--> 트랜잭션이 완료되기 전까지 데이터를 조작하는 세선(SESSION) 외에
...
-- > 서로 다른 행을 조작하면 LOCK이 발생이 안한다.

UPDATE TCL_수강생정보
SET 소속반 = 'D'
WHERE 학생ID = 'SMHRD9' ; 

-- > 두개의 세션이 접속을 했을때 서로 다른 세션에서 같은 행을 조작을 하게 되면
