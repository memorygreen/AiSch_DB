...
-- 직원테이블의 직원ID의 행의 개수를 조회 (총 직원수)
SELECT COUNT(EMPLOYEE_ID)
  FROM EMPLOYEES ;
--GROUP BY() ; 

-- 직원테이블을 하나의 그룹으로 묶은걸로 인식을 해서 그룹바이가 된 상태이다. 


-- 부서테이블의 부서ID의 행의 개수를 조회
SELECT COUNT(DEPARTMENT_ID)
  FROM DEPARTMENTS;


-- 직원테이블의 부서ID의 행의 개수를 조회
SELECT COUNT(DEPARTMENT_ID)
  FROM EMPLOYEES;
  
-- COUNT함수는 *(아스타리스크) 쓸수 있는 특징을 가지고 있습니다.
-- 다만 다른 함수는 쓸수가 없다.
...
-- [ GROUP BY 를 사용시 유의 사항 ]

-- 1. GROUP BY에 특정 컬럼을 사용하는 순간 행의 개수가 감소하기 때문에 GROUP BY보다 늦게 실행되는 절은
-- HAVING, SELECT, ORDER BY 에는 출력할 수 있는 행이 제한이 걸린다.
-- 왜냐? SQL 실행순서에 의해서

-- 2. 다만 집계함수로 처리하는 컬럼은 사용이 가능하다 .

-- 성적표 테이블에서 과목 별(GROUP BY) 최고 성적과 최저 성적을 출력
SELECT * FROM 성적표;
...
-- HAVING 절 : GROUP BY절이 존재 할 때만 사용이 가능하다.
