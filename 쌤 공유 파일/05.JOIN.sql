...
-- [ 등가조인의 사용방법 ]
-- 3. SELECT   테이블1.컬럼이름 , 테이블2.컬럼이름  --> 내가원하는 컬럼이름 .(경로)를 통해서 다 조회가능!
-- 1. FROM     테이블1 , 테이블2 --> 테이블1과 테이블2를 JOIN하겠다.
-- 2. WHERE    테이블1.컬럼이름 = 테이블2.컬럼이름 ; -- 조인 조건을 기술
...
-- 2. WHERE    일반 조건식을 따로 선별        --> 일반 조선절을 작성
...
-- 조인 조건은 대부분 PK와 FK 관계로 이루어진다 . 하지만 다 그런것은 아니다!!
...
-- 직원테이블의 총 행의 개수(107) * 부서테이블의 총 행의 갯수(27) ==> 2889개의 행이 나온것
-- 카티션 곱에 발생이 되는것이다 > 모든 경우의 수가 출력이 되는것을 말한다.
...
-- 조건이 직원테이블에 있는 매니저 아이디가 직원테이블에 있는 직원ID와 같은것 
...
--> 같은 값을 가지는 컬럼을 조인 조건으로 걸어줘야 한다.
--> 무조건 PK 와 FK 관계가 아니라 구하려는 값에 맞는 조인 조건을 걸어줘야 한다는 것!
