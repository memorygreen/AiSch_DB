...
-- 내장함수 : 오라클에서 미리 만들어 놓은 함수, 필요할 때마다 호출하여 사용
--> 문자형, 숫자형, 날짜형, 변환형, 형변환형, null처리함수, 조건함수

-- 함수이름() -> 함수를 실행할 때 사용하는 입력 값 : 인자 값, 매개변수
...
--> 임시 연산이나 함수의 결과값 확인 용도로 사용하는 테스트용 테이블이이다!!


-- 직원 테이블에서 직원의 FIRST_NAME, EMAIL 정보를 출력
...
-- 직원 테이블에서 직원의 FIRST_NAME 과 FIRST_NAME의 길이를 출력
...
--> 바꿔야할 문자를 생략 시 바꾸고 싶은 문자는 문자열 데이터에서 삭제가 된다.
...
-- TRIM() : 입력받은 문자형 데이터의 양 끝의 공백을 제거해주는 함수
SELECT TRIM('            즐   거   운    수   요   일          '),
       REPLACE(TRIM('            즐   거   운    수   요   일          '), ' '),
       REPLACE('            즐   거   운    수   요   일          ', ' ')
  FROM DUAL;
...
-- TURNC() : 특정 위치에서 버리는 함수
-- TRUNC(버림할 숫자, 버림 위치)
SELECT TRUNC(1234.56789, 2),
       TRUNC(1234.56789)
  FROM DUAL;
    
-- MOD() : 짝수를 나눈 나머지 값을 구하는 함수
...
-- TO_CHAR() : 날짜, 숫자 데이터를 문자데이터로 변환해주는 함수
...
SELECT TO_NUMBER('1,000', '9,999999') + 500
  FROM DUAL;
...
-- 1) NVL(NULL 인지 여부를 검사할 데이터, NULL일 경우 반환할 데이터)
...
-- 2) NVL2(NULL인지 여부를 검사할 데이터<1>,<1> 값이 NULL이 아닐 경우 반환할 데이터<2>,
...
-- DECODE(검사대상이 될 컬럼 또는 데이터(기준)<1>, <1>의 비교 값<2>,
